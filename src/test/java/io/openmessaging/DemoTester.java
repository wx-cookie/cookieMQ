            ...
            while ( (count = counter.getAndIncrement()) < maxMsgNum && System.currentTimeMillis() <= maxTimeStamp) {
                try {
                    ...
                    synchronized (queueCounter.get(queueName)) {
                        String msg = String.valueOf(queueCounter.get(queueName).getAndIncrement());
                        while(msg.length() < 58) {
                            msg += msg.toCharArray()[0];
                        }
                        queueStore.put(queueName, msg.getBytes());
                    }
                } catch (Throwable t) {
                    t.printStackTrace();
                    System.exit(-1);
        ...
        public void run() {
            ...
            while (counter.getAndIncrement() < maxMsgNum && System.currentTimeMillis() <= maxTimeStamp) {
                try {
                    ...
                    int index = random.nextInt(queueCounter.get(queueName).get()) - 21;
                    if (index < 0) index = 0;
                    Collection<byte[]> msgs = queueStore.get(queueName, index, 21);
                    int preIdx = index;
                    for (byte[] msg : msgs) {
                        String correctMsg = String.valueOf(index++);
                        while(correctMsg.length() < 58) {
                            correctMsg += correctMsg.toCharArray()[0];
                        }
                        if (!new String(msg).equals(correctMsg)) {
                            System.out.println("Check error " + queueName + " " + (index - 1) + " " + new String(msg) + " " + correctMsg + " " + preIdx);
                            System.exit(-1);
                        }
                    }
                } catch (Throwable t) {
            ...
            while (pullOffsets.size() > 0 && System.currentTimeMillis() <= maxTimeStamp) {
                try {
                    for (String queueName : pullOffsets.keySet()) {
                        ...
                        if (msgs == null || msgs.size() < 21) {
                            if (pullOffsets.get(queueName).get() != offsets.get(queueName).get()) {
                                System.out.println("Queue Number Error " + queueName + " " + preIdx + " " + (msgs == null? null: msgs.size()) + " " +
                                        pullOffsets.get(queueName).get() + " " + offsets.get(queueName).get());
                                System.exit(-1);
                            }
